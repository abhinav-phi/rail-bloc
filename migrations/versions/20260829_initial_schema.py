...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP SCHEMA IF EXISTS optimization CASCADE;"))
    op.execute(sa.text("DROP SCHEMA IF EXISTS operations CASCADE;"))
    op.execute(sa.text("DROP SCHEMA IF EXISTS demands CASCADE;"))
    op.execute(sa.text("DROP SCHEMA IF EXISTS infrastructure CASCADE;"))
