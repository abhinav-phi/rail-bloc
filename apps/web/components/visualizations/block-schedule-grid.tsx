...
const MOCK_BLOCKS = [
  {
    id: 'BLK-402',
    dept: 'CIVIL' as Department,
    section: 'DLI-PNP',
    dayIdx: 0,
    status: 'APPROVED_SR_DOM',
    duration: '10:00 - 14:00',
  },
  {
    id: 'BLK-415',
    dept: 'TRD' as Department,
    section: 'UMB-LDH',
    dayIdx: 2,
    status: 'DRAFT',
        ...
        <div className="grid grid-cols-[120px_1fr]">
          ...
          <div className="flex flex-col border-r bg-muted/10 sticky left-0 z-10">
            {SECTIONS.map((section) => (
              <div
                key={section}
                className="h-32 border-b flex items-center justify-center font-mono text-sm font-medium"
              >
                {section}
              </div>
            ))}
          ...
          <div className="relative">
            ...
            {SECTIONS.map((_, rIdx) => (
              <div
                key={`row-${rIdx}`}
                className="grid grid-cols-7 h-32 border-b"
              >
                {DAYS.map((_, cIdx) => (
                  <div
                    key={`cell-${rIdx}-${cIdx}`}
                    className="border-r last:border-r-0 hover:bg-accent/10 transition-colors relative"
                  />
                ))}
              </div>
            ))}
