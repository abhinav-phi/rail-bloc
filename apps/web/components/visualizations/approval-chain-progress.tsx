...
export function ApprovalChainProgress({
  ...
  return (
    <div className="relative pt-6 pb-2">
      ...
      <div
        className="absolute top-9 left-4 h-0.5 bg-primary transition-all"
        style={{ width: `${(resolvedIdx / (CHAIN_STAGES.length - 1)) * 100}%` }}
      />
      ...
      <div className="relative flex justify-between">
        {CHAIN_STAGES.map((stage, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`rounded-full bg-background p-1 ${isCompleted ? 'text-primary' : 'text-muted-foreground'}`}
              >
                {isCompleted ? (
              ...
              <span
                className={`text-xs text-center font-medium ${isCurrent ? 'text-foreground' : 'text-muted-foreground'}`}
              >
                {stage.label}
              </span>
            </div>
