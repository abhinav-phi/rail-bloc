...
FOR EACH ROW
EXECUTE FUNCTION fn_seal_ledger_entry();
...
-- silently forking the chain under concurrent writers (reproduced empirically with
-- 8 concurrent writer processes before this fix). Acquiring the lock in a SEPARATE,
-- EARLIER statement guarantees the subsequent INSERT statement's fresh snapshot
-- already contains every committed predecessor. All application writers MUST go
-- through audit.append_event(); the trigger remains as defense-in-depth.
