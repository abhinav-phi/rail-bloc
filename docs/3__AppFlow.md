...
7. **Non-selected candidates:** the emergency solver may produce multiple candidate plans; only the lowest-objective candidate is persisted as `PROVISIONAL`. Demands from non-selected candidates remain at `SUBMITTED` and are reconsidered in the next routine weekly solve cycle. This is intentional — it avoids persisting sub-optimal emergency schedules that would each require separate supersession and re-approval.
