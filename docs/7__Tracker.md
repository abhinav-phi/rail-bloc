...
| TASK-022 | Sentinel | Eng A | [x] | Fail-closed interceptor with the 3-attempt retry cap before `FAILED_ESCALATE_HUMAN` (FSM-002). **Runtime evidence 2026-09-09 (Railway deployment):** `solver_runs.stats.attempt_trace` recorded `attempts: 3` hitting the cap → run `FAILED` → 74 demands escalated `ESCALATED_OVERDUE`; the post-fix run (commit 586c07e) completed first-attempt FEASIBLE with 53 plans committed. |
...
- [ ] **Optimization Execution:** Solver converges on a constraint-verified weekly divisional plan in $\le 35$ **seconds (p95)** — **corrected from the previous "≤30 seconds" claim, which conflicted with NFR-001's authoritative ≤35s p95 figure (DOC-002).** *Measured on the live Railway deployment 2026-09-09 (N=10 weekly solves, hardware: Railway 2 vCPU shared, ML-ON; walls = `created_at`→`completed_at` from `optimization.solver_runs`): **single-attempt p95 ≤35 s NOT met on Railway** — walls 114.3–184.1 s (median ≈118.9 s, p95 nearest-rank 184.1 s), all 10 first-attempt COMPLETED, 0 FAILED; per-attempt budget enforcement is separate (`SOLVER_MAX_TIME_SECONDS=60` on Railway) and `stats.solver_wall_s` records the CP-SAT solve slice. Dev-hardware reference: 8.2 s single broker-driven solve (2026-09-05, TASK-020). NFR-001 stays open until a ≤35 s p95 is measured on the target hardware or the budget is re-baselined.*
...
| PERF-003 String Chart FPS (measured 2026-09-05, in-app Chromium, 1280×720, 8 s scroll/pan load, 1075 rAF samples) | **median ≈ 145 fps (median frame Δ 6.9 ms, p95 Δ 7.0 ms)** — exceeds the NFR-006 60 fps target; measured on dev-class hardware, figure to be re-cited per environment. *Re-verified 2026-09-06 after the real-timetable wiring: the chart renders 23 real train paths + striped plan blocks with no frame stalls (in-app Chromium caps rAF sampling in background tabs, so the second pass recorded idle-state cadence only — the load figure stands from the first measurement).* | rAF sampling script during canvas pan (TASK-061 session) |
...
- **[/] implemented, runtime-evidence pending:** TASK-039 (field endpoints tested via lifecycle activate/fitness; dedicated mobile-terminal mock flow pending), TASK-058 (per-solve wall times recorded in `solver_runs.stats`; GiST indexes present; FPS profiling done 2026-09-05 — PERF-003; re-cite per environment).
- **[x] TASK-022 runtime evidence captured (2026-09-09, Railway deployment):** the 3-attempt retry cap and `FAILED_ESCALATE_HUMAN` terminal state were observed live — `solver_runs.stats.attempt_trace` shows `attempts: 3` with the cap hit, run marked `FAILED`, 74 demands escalated to `ESCALATED_OVERDUE`; a post-fix run then completed first-attempt FEASIBLE. See `docs/DEPLOYMENT.md` and commit 586c07e.
- **[x] TASK-060 fully closed 2026-09-09:** dress rehearsal (2026-09-05) + public-deployment re-verification (53 plans via public URL, SSE frames through the Vercel rewrite, ledger 64/64) — evidence in `docs/DEPLOYMENT.md`.
