...
- [ ] **Optimization Execution:** Solver converges on a constraint-verified weekly divisional plan in $\le 35$ **seconds (p95)** — **corrected from the previous "≤30 seconds" claim, which conflicted with NFR-001's authoritative ≤35s p95 figure (DOC-002).** *Honest measurement status 2026-09-09: p95 is still NOT measured — what exists is single-run evidence: 8.2 s broker-driven solve on dev hardware (2026-09-05, TASK-020), a 100.1 s Railway wall-clock run that consumed all 3 Sentinel retry attempts (pre-fix), and a first-attempt FEASIBLE public-URL run post-fix (2026-09-09). A p95 across N runs remains open.*
